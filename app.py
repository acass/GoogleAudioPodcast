...
peaker 1: A fantastic overview. It sounds like Thread-X is set to make a real impact""")
    ...
    for chunk in client.models.generate_content_stream(
        ...
        if chunk.candidates[0].content.parts[0].inline_data and chunk.candidates[0].content.parts[0].inline_data.data:
            ...
            print(f"Received audio chunk {len(audio_chunks)}")
        else:
            print(chunk.text)

    # Combine all audio chunks and save as single file
    if audio_chunks:
        combined_audio = b''.join(audio_chunks)
        file_name = "tech_unraveled_podcast"
        ...
        save_binary_file(f"{file_name}{file_extension}", combined_audio)
        print(f"Created complete podcast with {len(audio_chunks)} audio chunks")
    else:
...
def parse_audio_mime_type(mime_type: str) -> dict[str, int]:
    ...
    Returns:
        A dictionary with "bits_per_sample" and "rate" keys as integers.
    """
    bits_per_sample = 16
    rate = 24000
    ...
    # Extract bits per sample from main type (e.g., "audio/L16")
